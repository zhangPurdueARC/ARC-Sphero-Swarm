        ...
        while True:
            ...
            while True and commands[id] != []:
                ...
                if (commands[id][0] == "%"): # be very careful with this - needs to be the absolute last command!
                    ...
                    distance = float(str(commands[id][0][1:]))
                    timeToTravel = 0.0
                    accDistance = 5.10 

                    if (distance < accDistance):
                        timeToTravel = (2 * distance / accDistance) 
                    elif (distance > accDistance):
                        timeToTravel = ((distance - accDistance) / accDistance) + 2 
                    api.roll(magBase + direction, 100, timeToTravel)  
...
# can't use normal find toy in conjunction "SB-76B3", "SB-1840", "SB-B11D"
